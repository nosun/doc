...
- conf 文件
		...
		a1.sources.r1.channels = c1
		a1.sources.r1.spoolDir = /data/log/logs
		
		# Describe the sink
		#a1.sinks.k1.type = logger
		#a1.sinks.k1.maxBytesToLog=1028
		a1.sinks.k1.type = hdfs
		a1.sinks.k1.channel = c1
		a1.sinks.k1.hdfs.path=/flume/evnets/
...
- 运行flume 
		...
		bin/flume-ng agent --conf conf --conf-file conf/device_log.conf --name a1 -Dflume.root.logger=INFO,console

- 查询hdfs文件

		hadoop dfs -ls /flume/events/
		hadoop dfs -cat /flume/envents/...

### hive

#### hive安装
- 下载并解压缩hive.1.2.1.bin
- 设置hive path 并expose
- 配置hive-site.xml,使用本地模式，mysql
...
- 添加数据表，tcp_logs
	...
	create table tcp_logs (
		...
		v9     string,
		v10    string,
	) partitioned by(date string) location '/data/hive' row format delimited fields terminated by ',';
